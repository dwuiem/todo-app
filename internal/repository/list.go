...
func (r *ListRepository) Delete(userID int, listID int) error {
	query := fmt.Sprintf("DELETE FROM %s tl USING %s ul WHERE tl.id = ul.list_id AND ul.user_id = $1 AND ul.list_id = $2", postgres.ListsTable, postgres.UsersListsTable)
	_, err := r.db.Exec(query, userID, listID)
	return err
}

func (r *ListRepository) Update(userID int, listID int, input model.UpdateListInput) error {
